...
// Ensure immediate update of uniform when texture loads
...
onBeforeRender(({ delta }: any) => {
	...
	if (live) {
		if (live[0]) targetColor1.value.set(live[0])
		if (live[1]) targetColor2.value.set(live[1])
	}
